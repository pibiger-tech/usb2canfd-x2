...
*/
    system("sudo ifconfig can1 down");// must close can device before set baud rate!
    //below mean depend on iprout tools ,not ip tool with busybox!
	 system("sudo ip link set can1 up type can bitrate 50000 dbitrate 2000000 fd on");
    //system("sudo echo 1000000 > /sys/class/net/can0/can_bittiming/bitrate");
    system("sudo ifconfig can1 up");
    printf("This is a socket canfd transmit demo program ,can1 with Bit Rate 500 kBit/s,Data Bit Rate 2MBit/s\r\n");
